...
############################## SSH key generation for ArgoCD ###################
print_message "nnl" "g" "Generating SSH key for ArgoCD | "
ssh-keygen -t ed25519 -f ~/.ssh/argocd_gitlab -C "argocd@cluster" -N ""
response_status $? "There was a problem generating the SSH key"


############################## Gitlab user creation via Rails ###################
print_message "nnl" "g" "Waiting for Gitlab to be ready | "
kubectl rollout status deployment/gitlab-webservice-default -n gitlab --timeout=1800s >> ./script.log 2>&1
response_status $? "Gitlab webservice did not become ready in 30 minutes"

# Copy ssh key inside gitlab webservice pod
kubectl cp ~/.ssh/argocd_gitlab.pub gitlab/gitlab-webservice-default:/tmp/argocd_gitlab.pub

# TODO gitlab-webservice-default not sure, get right pod with   ||
# kubectl get pods -n gitlab | grep toolbox | awk '{print $1}'  vv

# Create gitlab user with ssh key
kubectl exec -n gitlab deployment/gitlab-webservice-default -- \
  gitlab-rails runner "u = User.create!(username: 'vicalvez', email: 'vicalvez@local.com', name: 'Virgil Calvez', password: 'superpassword', skip_confirmation: true); u.save!"

############################## Gitlab repo creation ###################
print_message "nnl" "g" "Creating iot-repo repository | "
PUB_KEY="$(cat ~/.ssh/argocd_gitlab.pub)"

# TODO gitlab-webservice-default not sure, get right pod with   ||
# kubectl get pods -n gitlab | grep toolbox | awk '{print $1}'  vv

kubectl exec -n gitlab deployment/gitlab-webservice-default -- \
  gitlab-rails runner "u = User.find_by(username: 'vicalvez'); Key.create!(title: 'argocd_key', key: '$PUB_KEY', user: u)"


############################## Push initial content ###################
print_message "nnl" "g" "Cloning repo and pushing manifests | "
ssh-keyscan -H gitlab.local >> ~/.ssh/known_hosts
GIT_SSH_COMMAND="ssh -i ~/.ssh/argocd_gitlab" git clone git@gitlab.local:vicalvez/iot-repo.git
cd iot-repo
mkdir -p bonus/confs/manifests/
cp -r ../confs/manifests/* bonus/confs/manifests/
git add .
git commit -m 'Initial commit'
git push origin main


############################## ArgoCD repo add via SSH ###################
print_message "nnl" "g" "Adding GitLab repo to ArgoCD | "
argocd repo add git@gitlab.local:vicalvez/iot-repo.git \
    --ssh-private-key-path ~/.ssh/argocd_gitlab
...
argocd app create app \
  --repo git@gitlab.local:vicalvez/iot-repo.git \
  --path bonus/confs/manifests \
